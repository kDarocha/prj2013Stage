 ...
 */
function icebusiness_theme(&$existing, $type, $theme, $path) {
...
function icebusiness_preprocess(&$vars, $hook) {
 ...
 */
function icebusiness_preprocess_page(&$vars, $hook) {
...
function icebusiness_header_menu($element, $name) {
  $output = '';
  if ($element) {
...
function icebusiness_preprocess_node(&$vars, $hook) {
  ...
  // icebusiness_preprocess_node_page() or icebusiness_preprocess_node_story().
  $function = __FUNCTION__ . '_' . $vars['node']->type;
  if (function_exists($function)) {
    $function($vars, $hook);
  }
 if(!strstr($vars['title'],'nice-menu')){
	$titles=split(" ", $vars['title']);
 ...
 */
/* -- Delete this line if you want to use this function
function icebusiness_preprocess_comment(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
...
function icebusiness_preprocess_block(&$vars, $hook) {
 //print '<pre>';print_r($vars['title']);print'</pre>';
$titles=split(" ", $vars['title']);
  foreach ($titles as $k=>$v){
		if(($k%2)){
				$new_title.='<span class="blue">'.$v.'</span>&nbsp;';
